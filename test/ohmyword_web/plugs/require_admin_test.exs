defmodule OhmywordWeb.Plugs.RequireAdminTest do
  use OhmywordWeb.ConnCase, async: true

  alias OhmywordWeb.Plugs.RequireAdmin
  ...
  describe "call/2" do
    test "allows admin users through", %{conn: conn} do
      user = user_fixture()
      {:ok, admin_user} = Ecto.Changeset.change(user, role: :admin) |> Ohmyword.Repo.update()
